...
module.exports = function transformer(file, api) {
  ...
  let blocksHavingRunLoopExpressions = secondRoot
    .find(j.CallExpression, {
      callee: {
        name: 'run',
      },
  ...
  blocksHavingRunLoopExpressions.forEach((nodePath) => {
    ...
    node.body.forEach((statement) => {
      if (statement.expression && statement.expression.callee && statement.expression.callee.name === 'run') {
        exArr.push(true);
      } else {
        exArr.push(false);
      }
  ...
  thirdRoot
    ...
    .forEach((nodePath) => {
      ...
      let newDeclaration = j.variableDeclaration(declarationType, [
        j.variableDeclarator(j.identifier(declaration.id.name), null),
      ]);
      ...
      j(nodePath).replaceWith(
        j.expressionStatement(j.assignmentExpression('=', j.identifier(variableName), declaration.init))
      );
    });
  ...
  if (isModified && !isImportExists) {
    ...
    thirdRoot.find(j.ImportDeclaration).get().insertAfter(importStatement);
  }

  function isRunLoop(path) {
    return path.value.expression && path.value.expression.callee && path.value.expression.callee.name == 'run';
  }
  ...
  function wrapRunLoop(nodePath) {
    ...
    const newNode = j.expressionStatement(j.callExpression(j.identifier('run'), [arrow]));
    isModified = true;
    return newNode;
  }
