...
module.exports = function transformer(file, api) {
  ...
  const setupTestTypes = ['setupTest', 'setupRenderingTest', 'setupApplicationTest'];

  const removeTypes = ['setupRenderingWithMirage', 'setupAcceptance'];
  ...
  function removeUnused(root) {
    root
      ...
      .filter((path) => {
        ...
        let calledImport = root.find(j.CallExpression, { callee: { name } }).filter((path) => {
