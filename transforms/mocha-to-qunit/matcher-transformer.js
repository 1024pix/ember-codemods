...
module.exports = [
  ...
  */
    matcher: function (expression) {
      return (
        expression.property &&
        ['ok', 'empty', 'exist', 'present', 'undefined', 'null', 'nil'].includes(expression.property.name)
      );
    },
    transformer: function (expression, path, j) {
      var { assertArgumentSource, assertArgument, assertMessage, hasShouldNot, hasSelectorWithoutProperty } =
  ...
  */
    matcher: function (expression) {
      return (
        expression.callee &&
        ['lt', 'below', 'lte', 'gt', 'above', 'gte', 'least', 'most'].includes(expression.callee.property.name)
      );
    },
    transformer: function (expression, path, j) {
      var { assertArgumentSource, assertMessage } = extractExpect(path, j);
  ...
  },
  {
    ...
    matcher: function (expression) {
      return (
        (expression.callee &&
          ['attr', 'attribute', 'class', 'text', 'value', 'prop'].includes(expression.callee.property.name)) ||
        (expression.property && ['visible', 'disabled', 'enabled', 'displayed'].includes(expression.property.name))
      );
    },
    transformer: function (expression, path, j) {
      var { assertArgument, assertMessage, hasShouldNot, hasSelector, hasSelectorWithoutProperty } = extractExpect(
        path,
        j
      );
  ...
  },
  {
    ...
    transformer: function (expression, path, j) {
      ...
      let expectedArgument =
        expression.callee.property.name === 'members'
          ? j(expression.arguments).toSource()
          : `[${j(expression.arguments).toSource()}]`;
  ...
  },
  {
    ...
    transformer: function (expression, path, j) {
      ...
      let assertArgs = hasShouldNot
        ? joinParams(assertArgumentSource, assertMessage)
