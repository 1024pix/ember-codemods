...
const { cleanupImports, setupHooksForTest, setupCallbackHooks, removeChaiImports } = require('../cleanup-imports');
const beautifyImports = require('../beautify-imports');
const { findExpect, findSinonAssert, renameIdentifiers, renameImports } = require('./utils');
...
module.exports = function transformer(file, api) {
  ...
  function removeDoneMethod(path) {
    j(path).find(j.Identifier, { name: 'done' }).closest(j.AwaitExpression).remove();
    j(path).find(j.Identifier, { name: 'done' }).closest(j.CallExpression).remove();
  }
  ...
  function transformerTests(path) {
    ...
    filterExpressions(j(path).find(j.ExpressionStatement)).forEach(transformExpect);

    filterExpressions(j(path).find(j.ReturnStatement)).forEach(returnExtraction);
  }

  function filterExpressions(graphTree) {
    return graphTree.filter(pathHasExpects).filter(filterOnlyFunctions);
  }
  ...
  function runMacherTranformer(expression, path) {
    ...
    try {
      matcherTransformer.forEach(({ name, matcher, transformer }) => {
  ...
  function addAssertOkAfterSinonAssert(j, root) {
    ...
    memberExpressions.forEach((memberExpression) => {
      ...
      const founded = j(blockExpression).find(j.MemberExpression, {
        object: { name: 'assert' },
        property: { name: 'ok' },
      });
      if (founded.length !== 0) {
      ...
      const assertCall = j.expressionStatement(
        j.callExpression(j.memberExpression(j.identifier('assert'), j.identifier('ok')), [j.identifier('true')])
      );
  ...
  function replaceExpect(root, j) {
    root
      ...
      .replaceWith((path) => {
        return j.callExpression(j.memberExpression(j.identifier('assert'), j.identifier('ok')), path.node.arguments);
      });
  }
  ...
  function replaceTimeout(j, root) {
    root
      ...
      .replaceWith(j.memberExpression(j.identifier('assert'), j.identifier('timeout')));
  }
  ...
  function transformerMissingTests(path) {
    ...
    j(path).find(j.ExpressionStatement).filter(pathHasExpects).forEach(transformExpect);

    j(path).find(j.ReturnStatement).filter(pathHasExpects).forEach(returnExtraction);
  }
